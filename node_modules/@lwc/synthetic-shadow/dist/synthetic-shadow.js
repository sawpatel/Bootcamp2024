 ...
 * For full license text, see the LICENSE file in the repo root or https://opensource.org/licenses/MIT
 */
const currentlyHandledEvents = new Set();
function isBeingHandledByWrappedListener(event) {
  return SetHas.call(currentlyHandledEvents, event);
}
function invokeWrappedListener(target, listener, event) {
...
defineProperty$1(Node.prototype, KEY__SHADOW_RESOLVER, {
  set(fn) {
...
defineProperty$1(_globalThis$1, KEY__IS_NATIVE_SHADOW_ROOT_DEFINED, {
...
SyntheticShadowRoot.prototype = create$1(DocumentFragment.prototype, SyntheticShadowRootDescriptors); // `this.shadowRoot instanceof ShadowRoot` should evaluate to true even for synthetic shadow

defineProperty$1(SyntheticShadowRoot, Symbol.hasInstance, {
  value: function (object) {
    ...
    return isObject(object) && !isNull(object) && (isInstanceOfNativeShadowRoot(object) || getPrototypeOf$1(object) === SyntheticShadowRoot.prototype);
  }
});
/**
...
function patchedTargetGetter() {
  ...
  if (!runtimeFlags.ENABLE_RETARGETING_FOR_UNPATCHED_LISTENERS) {
  ...
  const composedPath = pathComposer(originalTarget, this.composed); // Handle cases where the currentTarget is null (for async events), and when an event has been
  // added to Window
...
defineProperty$1(Element.prototype, KEY__SHADOW_TOKEN, {
  set(shadowToken) {
